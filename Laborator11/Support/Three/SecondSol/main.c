...
int main() {
	...
	if(pid1 == 0) { 
		...
		{ // Secțiune de cod executată doar de către "nepotul" procesului inițial.
			...
			perror("Eroare la execlp() in al doilea proces fiu. Cauza erorii");  exit(6);
		}
		else
		{ // Secțiune de cod executată doar de către fiul procesului inițial.
			close(pipe_Cmd1toCmd2[1]); // E necesar acest close pentru ca să se termine execuția comenzii sort (prin "citirea" EOF din canal).
	...
	{ // Secțiune de cod executată doar de către părinte (i.e., procesul inițial).
		close(pipe_Cmd2toCmd3[1]); // E necesar acest close pentru ca să se termine execuția comenzii sort (prin "citirea" EOF din canal).
