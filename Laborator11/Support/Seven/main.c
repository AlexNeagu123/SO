...
int main()
{
	...
	{
		if(errno != EEXIST) {  
			// errno=17 for "File already exists"
	...
	if(pid1 == 0)
	{
		...
		if(pid2 == 0)
		{
			if(-1 == (fdo = open("fifo_Cmd1toCmd2", O_WRONLY))) {  
				perror("Eroare: apelul open in al doilea proces fiu a esuat. Cauza erorii");  
				exit(5);  
			}
			if(-1 == dup2(fdo,1)) {  
		...
		else
		{
			if(-1 == (fdi = open("fifo_Cmd1toCmd2", O_RDONLY))) {  
				perror("Eroare: apelul open in primul fiu a esuat. Cauza erorii");  
				exit(8);  
			}
			...
			if(-1 == (fdo = open("fifo_Cmd2toCmd3", O_WRONLY))) {  
				perror("Eroare: apelul open in primul proces fiu a esuat. Cauza erorii");  
				exit(10);  
			}
	...
	else
	{
		if(-1 == (fdi = open("fifo_Cmd2toCmd3", O_RDONLY))) {  
			perror("Eroare: apelul open in parinte a esuat. Cauza erorii");  
			exit(13);  
		}
