/*
  Program: șablonul de creare a unei liste de N procese, varianta iterativă.
*/
#include <unistd.h>
#include <sys/wait.h>
#include <sys/types.h>
    ...
    for(i = 1; i < N; i++)
    {
        ...
        if(0 != pid)
        {
            wait(&codterm); // Aici fiecare proces va aștepta terminarea procesului fiu direct, ca să-i obțină codul de terminare
                /* No./tă: chiar dacă nu se cerea afișarea codului de terminare, tot ar fi nevoie aici de un apel wait(NULL); (!)
